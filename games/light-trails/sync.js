...
export class GameSync {
  ...
  reset() {
    ...
    this.cursor = null; this.frameAt = null; this.waiting = false;
  }
  ...
  inputs() { return [...array(this.state?.players[this.own]?.inputs), ...this.pending]; }
  enqueue(heading) {
    const p = this.state?.players[this.own];
    if (!p || !['playing','countdown'].includes(this.state.phase)) return null;
    const inputs = this.inputs(), direction = inputs.at(-1)?.heading ?? p.dir;
    if (inputs.length >= 6 || heading === direction || (heading - direction + 4) % 4 === 2) return null;
    const input = {seq:++this.seq, heading}; this.pending.push(input); return input;
  }
  ...
  project(time) {
    ...
    this.frameAt = time;
    // Recover from an exhausted buffer through bounded playback, not a position jump.
    const speed = 1 + clamp((desired - this.cursor) / 400, -.1, .15);
    this.cursor = Math.min(limit, this.cursor + dt * speed);
    this.waiting = desired > limit + 30 && this.cursor >= limit - .01;
    return sampleCommitted(state, this.cursor);
  }
}
...
export function sampleCommitted(state, time) {
  ...
  for (let n=0;n<Math.ceil(ahead);n++) {
    ...
    if ((moves[0].x===moves[1].x && moves[0].y===moves[1].y) || (moves[0].x===players[1].x && moves[0].y===players[1].y && moves[1].x===players[0].x && moves[1].y===players[0].y)) {
